function  h = imshow3D_filter( Img, varargin)
...
p = inputParser;
p.addParameter('zval',@isinteger);
p.addParameter('initial_thresh',prctile(vals,90),@isinteger);
p.addParameter('filter_on',false,@islogical);
p.parse(varargin{:})

sno = size(Img,3);  % number of slices
if isfield(p.Results,"zval")
    S = p.Results.zval;
else
    S = round(sno/2);
end

initial_thresh = p.Results.initial_thresh;
...
h = axes('position',[0,0.2,1,0.8]), imshow(squeeze(Img(:,:,S,:)), [Rmin Rmax])
...
ChBxFilter_hand.Value=p.Results.filter_on;
