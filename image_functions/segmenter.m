...
classdef segmenter < handle  
    ...
    methods (Access=public)
        ...
        function pre_smooth(obj)
            obj.img = imgaussfilt3(app.S.img, obj.params.smooth_sigma);
        end
        ...
        function spot_threshold_simple(obj, cell_centroids)
            ...
            obj.BW = obj.filtered >= params.spot_threshold;
            ...
            [app.spot_stats.assignment] = A{:};
            
            
            app.add_step('simple_spot_threshold');
        ...
        function spot_threshold_percentile(obj)
                    ...
                    params.percentiles = [app.spot_percentile_img.Value, app.spot_percentile_filtered.Value];
                    
                    % Set 'indices' to all.
                    indices=1:length(app.frame_obj.(app.cell_mask_channel_str).centroids);

                    frame_obj = app.frame_obj.(app.cell_mask_channel_str);
                    [app.spot_stats, ~] = local_threshold_cells( app.S.img, app.S.filtered, frame_obj, indices, app.seg_params.(app.channel_str).spots );
                    app.S.BW = false(size(app.frame_obj.BW));
                    app.S.BW( [app.spot_stats(:).PixelIdxList] ) = 1;
                    app.add_step('local_spot_threshold');
        end
