% tStart = tic;
function [nuclei_centroid_voxels_microns, nuclei_centroid_voxels_microns_gmm, nuclei_centroid_voxels_microns_kmeans] = DS_GMM_Kmeans_3dnucleisegmentation_LSM(in_image, im_info)
...
%% Reads input file im_information
...
%     image_bits=inputfile(1,1).bits;
    ...
    %Input image should be a z-stack with data field having raw intensities
    
    ZSlicesinStack = im_info.Z;
    xscale = im_info.PixelSize(1);
    yscale = im_info.PixelSize(2);
    zscale = im_info.PixelSize(3);
    image_bits = im_info.BitSize;
...
%     Image_details=regexp(inputfile(1,1).im_info,'\n','split');
...
for time=1%TotalTime %% iterate through time points
    for zslice= 1:ZSlicesinStack %% iterate through number of slices
        ...
        I(:,:,zslice)=in_image(zslice).data;  %% Reads one z-slice in the stack
    ...
    if fused_nuclei
        ...
        for l=1:length(liar_list)
            ...
            Active_GMM=0; %By setting to zero, you can switch off GMM
            if Active_GMM
            ...
            Active_Kmeans=0; %By setting to zero, you can switch off Kmeans
