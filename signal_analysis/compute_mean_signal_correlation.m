function mean_sigcorr = compute_mean_signal_correlation( sigcorr, indices )
...
if nargin < 2 || isempty(indices)
    indices = find(~cellfun('isempty',sigcorr));
    
    if length(indices) < length(sigcorr)
        disp('found some empty correlations...')
    end
end
...
mean_sigcorr = [ delays mmean mstd (sum_weight.^2 ./ sum_square_weight) ];
