...
%%%%%%%%%%%%%%% START PROCESSING %%%%%%%%%%%%%%%%
    ...
    %Collect zero-pixel indices. Later we can just set to zero in BW
    %because sometimes these get set to foreground cause large gradient. 
    bg_ind = find( I == 0);
    ...
    if step.local_hist_thresh
        ...
        f = @(x) estimate_local_threshold( x, params );
        thrsh_img = blockproc3(J,params.block_size,f);
        thrsh_img = imgaussfilt3(  thrsh_img, params.block_size,'filterdomain','spatial');
    end
