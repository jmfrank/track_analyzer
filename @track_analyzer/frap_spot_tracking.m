...
function obj = frap_spot_tracking(obj, params)
...
IMG = zeros(length(rows),length(cols),pre_bleach_T);
for t = 1:pre_bleach_T
    %one channel image, get time. 
...
%% For the pre-bleach frame, we know the spot is totally gone. Open post-bleach image and measure the intensity where the spot was. 
reader = bfGetReader( obj.exp_info.img_file );
stack = get_stack( reader, 1, 1);
%Project. 
...
for t = 2:T
    ...
    if(isempty(fit))
       ...
       %Position vector. comes as y,x, (z)
        ...
        xyzt = [xyzt; pos, ids', (t+pre_bleach_T).*ones(size(pos,1),1)];
