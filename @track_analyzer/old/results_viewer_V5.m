...
function obj = results_viewer_v5(obj, params,step,tracks)
...
if( iscell( obj.exp_info.img_file ))
    ...
    for e = 1:n_imgs
        ...
        for t = 1:T
            plane = get_planes(reader,Z,params.view_channel,t);
...
%Make empty text handle array. Index of entry corresponds to track id.
all_textH = gobjects(1);
%% TOOL box
...
%Drawing detected spot centroids on top of frame
    function DrawTracks(object, eventdata)
        ...
        %Get existing texts. Stored in global all_textH. Plotted track ids. 
        p_t_ids = find( isgraphics(all_textH));
        
        %Draw selected tracks as green and unselected as red. 
        hold on
        on_tracks = find( track_matrix(:,t) )';        
        %Add new text for those that are in on_track and not already
        %plotted, or just update position if already exists. 
        need_to_plot = setdiff(on_tracks,p_t_ids);
        for i = on_tracks
            this_track = tracks_curr{i};
            id = t - track_starts(i) + 1;
            POS = this_track(id,3:4);
            if any(need_to_plot==i)
                %Make clickable text object. 
                all_textH(i) = text(POS(1),POS(2),num2str(i),'color',c_vec(sel_vec(i)+1,:),'ButtonDownFcn',@textcallback,'UserData',i);
            else
                %No need to replot. Just update position and turn on.  
                all_textH(i).Position(1:2) = POS;
                all_textH(i).Visible='on';
            end
        end
        
        %Set visibility of tracks that are not in this plane. 
        to_be_invisible = setdiff(p_t_ids,on_tracks);
        if~isempty(to_be_invisible)
            set(all_textH(to_be_invisible),'Visible','off')
        end
...
%% Drawing spots
    function DrawSpots(states)
        ...
        for i = union(sel_tracks,on_tracks)
