...
% a different wrapper. This would improve usability. 

% 9-12-18: Need a better way to account for variable intensity besides
% CLAHE. Seems like an interative method for finding local maxima then
% estimating local background would work. 
...
%%%%%%%%%%%%%%% START PROCESSING %%%%%%%%%%%%%%%%
    ...
    for i = 1:ZSlicesinStack
        ...
        %end
        plane_mean(i) = mean(this_plane_img(~sel));
        plane_std(i)  = std(double(this_plane_img(~sel)));
        I(:,:,i)     = this_plane_img;
    end
    %Collect zero-pixel indices.
    bg_ind = find( I == 0);
    ...
    if step.local_hist_thresh
        
        %Function handle to histogram method. 
        f = @(block_struct) estimate_local_threshold( block_struct, params.hist_thresh, thrshlevel );
        thrsh_img = blockproc3(I,params.block_size,f);
